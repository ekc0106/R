...
for(l in 2:5){
  ...
  # 이 문제에서는 train에서 적합한 모델이 구간마다 식이 있는게 아닌, 각 eval.point마다의 적합된    점을 이은것이기 때문에(type='l') xte의 점에 대응되는 yhat_te값은 적합된점들의 내분점으로         구하겠다.
  for(i in 1:100){
    for(j in 1:100){
...
min_k
```
즉, 지난 과제에서 k값이 작을수록 data에 잘 적합하는 것을 볼 수 있었지만, 새로운 데이터 셋에 적합시키면 오히려 SSE가 커짐을 확인하였다. 즉 overfitting이 일어났음을 알 수 있다. seed 값에따라 다르지만, 4번의 반복시행결과 k=2,9,5,4일 때 SSE가 가장 작게 나왔다.